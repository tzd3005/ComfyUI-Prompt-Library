// ComfyUI Prompt Library V2 — 前端文本预览扩展
// 原理与 AlekPet 的 PreviewTextNode 一致：
//   - 后端返回 {"ui": {"string": [text]}}
//   - 前端在节点创建时用 ComfyWidgets.STRING 挂一个只读文本框 widget
//   - onExecuted 钩子把后端返回的 ui.string 写入该文本框
//
// 必须配合后端节点名 "ComfyUI_Prompt_Preview" 使用。

import { app } from "../../scripts/app.js";
import { ComfyWidgets } from "../../scripts/widgets.js";

app.registerExtension({
  name: "ComfyUI_Prompt_Library_V2.PromptPreview",

  async beforeRegisterNodeDef(nodeType, nodeData, app) {
    if (nodeData.name !== "ComfyUI_Prompt_Preview") {
      return;
    }
    ...
    nodeType.prototype.onNodeCreated = function () {
      ...
      const count = app.graph._nodes.filter(
        (n) => n.type === "ComfyUI_Prompt_Preview"
      ).length;
